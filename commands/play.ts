...
export const play = async (message: Discord.Message, args: string[], serverQueue: IQueue) => {
  await join(serverQueue)
  
  if (args[0].startsWith("https://www.youtube.com/")) await youtube(args[0], serverQueue)

  else if (args[0].startsWith("https://youtu.be/")) {
    const array = args[0].split('/')
    await youtube(`https://www.youtube.com/watch?v=${array[array.length - 1]}`, serverQueue)
  }
...
const search = async (q: string, serverQueue: IQueue, message: Discord.Message) => {
  var options = {
    q,
    part: 'snippet',
    type: 'video'
  ...
  try {
    ...
    for (let i = 0; i < res.items.length; i++) {
      exampleEmbed.addField(`${i+1}: ${res.items[i].snippet.title}`, res.items[i].snippet.channelTitle)
    }
...
export const searchSelect = async (id: string, serverQueue: IQueue, userId: string) => {
  if (!(id === 'cancel' || id === 'c')) {

    const videoId = serverQueue.finding.get(userId).res.items[+id - 1].id.videoId
